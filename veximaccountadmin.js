...
if (window.rcmail) {
	...
	rcmail.register_command('plugin.veximaccountadmin', function() { rcmail.goto_url('plugin.veximaccountadmin') }, true);
	rcmail.register_command('plugin.veximaccountadmin-save', function() { 
	var input_curpasswd = rcube_find_object('_curpasswd');
	var input_newpasswd = rcube_find_object('_newpasswd');
	var input_confpasswd = rcube_find_object('_confpasswd');
	var input_vacation = rcube_find_object('vacation');
	
	if (input_curpasswd && input_curpasswd.value=='' && input_newpasswd.value!='') {
		alert(rcmail.gettext('enterallpassfields', 'veximaccountadmin'));
		input_curpasswd.focus();
	} else if (input_curpasswd && input_curpasswd.value=='' && input_confpasswd.value!='') {
		alert(rcmail.gettext('enterallpassfields', 'veximaccountadmin'));
		input_curpasswd.focus();
	} else if (input_curpasswd && input_curpasswd.value!='' && input_newpasswd.value=='') {
		alert(rcmail.gettext('enterallpassfields', 'veximaccountadmin'));
	...
	//                http://roundcube.net/plugins/sauserprefs)
			...
			rcmail.register_command('plugin.veximaccountadmin.addressrule_del', function(props, obj){
				...
				if (!confirm(rcmail.gettext('headerblockdelete','veximaccountadmin')))
					return false;

				if (document.getElementsByName('_headerblock_rule_act[]')[fieldidx].value == "INSERT") {
			...
			rcmail.register_command('plugin.veximaccountadmin.headerblock_add', function(){
				var adrTable = rcube_find_object('headerblock-rules-table').tBodies[0];
				...
				if (input_headerblockvalue.value.replace(/^\s+|\s+$/g, '') == '') {
					alert(rcmail.gettext('headerblockentervalue','veximaccountadmin'));
					input_headerblockvalue.focus();
					return false;
				}
				else {
					var actions = document.getElementsByName('_headerblock_rule_act[]');
					var prefs = document.getElementsByName('_headerblock_rule_field[]');
					var addresses = document.getElementsByName('_headerblock_rule_value[]');
					var insHere;

					for (var i = 1; i < addresses.length; i++){
						if (prefs[i].value == input_headerblockrule.options[selrule].value && addresses[i].value == input_headerblockvalue.value && actions[i].value != "DELETE") {
							alert(rcmail.gettext('headerblockexists','veximaccountadmin'));
							input_headerblockvalue.focus();
							return false;
						}
			...
			rcmail.register_command('plugin.veximaccountadmin.headerblock_delete_all', function(props, obj){
				var adrTable = rcube_find_object('headerblock-rules-table').tBodies[0];

				if (!confirm(rcmail.gettext('headerblockdeleteall','veximaccountadmin')))
					return false;
