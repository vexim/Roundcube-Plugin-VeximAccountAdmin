...
class veximaccountadmin extends rcube_plugin
{
    ...
    function veximaccountadmin_form()
    {
        ...
        $table = new html_table(array('class' => 'headerwhiteprefstable','cols' => 3));
        $field_id = 'rcmfd_headerwhiterule';
        $input_headerwhiterule = new html_select(array('name' => '_headerwhiterule','id' => $field_id));
        ...
        $input_headerwhitevalue = new html_inputfield(array('name' => '_headerwhitevalue','id' => $field_id,'style' => 'width:270px;'));
        
        $field_id = 'rcmbtnwhite_add_address';
        $button_addaddress = $this->api->output->button(array('command' => 'plugin.veximaccountadmin.headerwhite_add','type' => 'input','class' => 'button','label' => 'veximaccountadmin.addwhiterule','style' => 'width: 130px;'));
        
        $table->add(null, $input_headerwhiterule->show());
        $table->add(null, $input_headerwhitevalue->show());
        $table->add(array('align' => 'right'), $button_addaddress);
        
        $delete_all = $this->api->output->button(array('command' => 'plugin.veximaccountadmin.headerwhite_delete_all','type' => 'link','label' => 'veximaccountadmin.deleteall'));
        
        $table->add(array('colspan' => 3,'id' => 'listcontrols'), $delete_all);
        $table->add_row();
        
        $address_table = new html_table(array('id' => 'headerwhite-rules-table','class' => 'records-table','cellspacing' => '0','cols' => 3));
        $address_table->add_header(array('width' => '120px'), $this->gettext('field'));
        $address_table->add_header(null, $this->gettext('value'));
        $address_table->add_header(array('width' => '40px'), '&nbsp;');
        ...
        $address_table->set_row_attribs(array('style' => $norules));
        $address_table->add(array('colspan' => '3'), rcube_utils::rep_specialchars_output($this->gettext('nowhiteaddressrules')));
        ...
        $table->add(array('colspan' => 3), html::div(array('id' => 'headerwhite-rules-cont'), $address_table->show()));
        $table->add_row();
    ...
    private function _whiteaddress_row($address_table, $field, $value)
    {
        ...
        $hidden_action = new html_hiddenfield(array('name' => '_headerwhite_rule_act[]','value' => ''));
        $hidden_field = new html_hiddenfield(array('name' => '_headerwhite_rule_field[]','value' => $field));
        $hidden_text = new html_hiddenfield(array('name' => '_headerwhite_rule_value[]','value' => $value));
        ...
        $del_button = $this->api->output->button(array('command' => 'plugin.veximaccountadmin.whiteaddressrule_del','type' => 'image','image' => 'plugins/veximaccountadmin/delete.png','alt' => 'delete','title' => 'delete'));
        $address_table->add('control', '&nbsp;' . $del_button . $hidden_action->show() . $hidden_field->show() . $hidden_text->show());
